...
5. **Hybrid-Entwicklung.** Claude Code baut nach diesem Framework; Yasin entscheidet, deployt und liefert die Daten (Screenshots, GSC), die die Maschine nicht beschaffen kann. Alles wird aus dem Framework abgeleitet — kein Feature ad-hoc.
...
**C** Bing Webmaster Tools (Yasin, einmalig) — Tor zu ChatGPT/Copilot-Index.
**D** Indexierung beschleunigen (Yasin: URL-Prüfung Top-20, Sitemap-Pflege).
**E** Reddit/Gutefrage-Antworten (Yasin, laufend) — **freigegeben, aktueller Fokus.**
...
**H** Bilder: Alt-Text-Pass + Mehrbild-Galerien pro Produkt (Bild-URLs liefert Yasin).
...
|---|---|---|
| **Yasin** | Prioritäten, Freigaben, Screenshots/GSC-Daten, Off-Site (Block C/D/E/F-Outreach), Live-Stichproben, Rollback (git revert) | Boilerplate |
| **Claude (Chat)** | Specs, Architektur-Review, Auswertung der GSC-Daten, Pushback | still zustimmen |
| **Claude Code** | Bau nach Spec+Constitution+Patterns, verify.py, STATUS-Pflege, Commit+Push (= Deploy via Actions) nach grünem verify | Architektur eigenmächtig ändern, roten Stand pushen |

## 7.3 Feature-Kette (verbindlich für jedes größere Vorhaben)
Idee → Spec (`06-specs/SPEC-<name>.md`: Ziel · Keyword-Bezug · Seiten/Änderungen · Gesetzes-Check · Verify-Gate) → Freigabe Yasin → Claude Code baut → verify.py grün + Review in frischem Kontext → Claude Code committet + pusht (Deploy automatisch via Actions) → **Rückfluss** (neues Pattern? Gesetz präzisieren? → Brain).
...
1 · Messbares Fertig-Kriterium (Maschine prüfbar). 2 · Macher ≠ Prüfer (Review frischer Kontext). 3 · State-Datei `04-loops/LOOP-STATE.md` — jeder Lauf liest sie zuerst, erledigte Arbeit wird nie wiederholt. 4 · Stop-Bedingung: 3 Versuche pro Item, dann blockiert loggen und weiter. 5 · Menschen-Gate: Geld/ASIN ohne Screenshot-Beleg, .github/workflows, .claude/settings.json → stoppen, auf „braucht Yasin"-Liste. Deploy läuft autonom über das verify-Gate (seit 18.07.2026). 6 · Kein Loop für einmalige/vage/nicht prüfbare Arbeit.
...
3. Der Mensch hat das letzte Wort: Richtung/Prioritäten, Geld/ASINs ohne Screenshot-Beleg, neue Domains, Workflow- und Permission-Änderungen, Rollback = Yasin.
...
5. **Keys, not prompts:** Sicherheit über Zugriffsrechte, nicht über Anweisungen. Seit 18.07.2026 deployt Claude Code autonom (Commit+Push nach grünem verify.py); die Schlüssel bleiben strukturell: der Actions-Workflow entscheidet, was öffentlich wird (brain/, scripts/, CLAUDE.md nie), .github/workflows/ und .claude/settings.json sind Menschen-Gates, und Yasin hält das Rollback-Recht via git revert.
...
| 2026-07-18 | 1.1 | Vollautonomie (Yasins Entscheidung): Claude Code committet/pusht selbst, verify.py als hartes Gate ersetzt das Deploy-Menschen-Gate (Governance 3+5, Rollenteilung 7.2, Feature-Kette 7.3, Loop-Regel 5). Namenskorrektur: Projektinhaber heißt Yasin; der bisher im Brain verwendete Rufname war ein Transkriptionsfehler und wurde projektweit ersetzt. |

---

*SPC FRAMEWORK v1.1 · Single Source of Truth · oberste Datei neben INDEX.md*
